...
<body>
  ...
  <!----VIỆC LÀM MỚI NHẤT------------------------------------------------------------------------------------------------------------>
        ...
        <h4 class="title">VIỆC LÀM MỚI NHẤT</h4>
            <?php
              $sql = "SELECT * 
                      FROM congviec cv join nganhnghe nn on cv.IDnn=nn.IDnn join luong l on l.IDluong=cv.IDluong join thanhpho tp on tp.IDtp=cv.IDtp join gioitinh gt on cv.IDgt=gt.IDgt order by cv.IDcv desc";
              ...
              if(mysqli_num_rows($result) > 0){
                ...
                while($row = mysqli_fetch_assoc($result)){    
                  echo "<div class='item'>";
                    ...
                    echo $row['tencv']."</br>";
                    echo "<i class='fas fa-map-marker'> &nbsp;Địa chỉ:&nbsp; </i> " .$row["tentp"] ."</br>";
                    echo "<i class='far fa-money-bill-alt' style='font-weight: bold'>&nbsp;Giá thuê:&nbsp; </i>" .$row["mucluong"] ."</br>";
                    echo "<i class='fas fa-user-circle'>&nbsp;Giới tính:&nbsp; </i>" .$row["tengt"] ."</br>";
                    echo "<td> <a href='./layouts/ChiTietcv.php?IDcv=".$row["IDcv"]."'> Chi tiết >> </a> </td>";
                  echo "</div>";
              }
              echo "<div class='item1'>";
                    echo "<a href='./ViecLamMoiNhat.php?nganh=QTKD'>+ Xem thêm việc làm mới nhất</a>";
                  echo "</div>";
              echo "</div>";
            }  
            ?>
            ...
            <?php
              ...
              if(mysqli_num_rows($result)>0)
              {
                ...
                while($row = mysqli_fetch_assoc($result)){
                  ...
                  echo "<div class='item1'>";
                    echo "<a href='./ViecLamCanGap.php?nganh=QTKD'>+ Xem thêm việc làm cần gấp</a>";
                  echo "</div>";
              echo "</div>";
            }
            ...
            <?php
              ...
              if(mysqli_num_rows($result)>0)
              {
                ...
                while($row = mysqli_fetch_assoc($result)){
                  ...
                  echo "<div class='item1'>";
                    echo "<a href='./ViecLamLuongCao.php?nganh=QTKD'>+ Xem thêm việc làm lương cao</a>";
                  echo "</div>";
              echo "</div>";
            }
